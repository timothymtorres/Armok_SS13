# The modularization handbook - Armok style, v0.2
...
And then you'll want to establish your core folder that you'll be working out of which is normally your module ID. E.g. `modular_armok/modules/shuttle_toggle`
...
We will no longer have _armok map versions.
...
- ***Example:*** You're adding a new lavaland mob.
  ...
  First of all you create your modular folder. E.g. `modular_armok/modules/lavalandmob`
  ...
  ```byond
    /mob/lavaland/newmob
      icon = 'modular_armok/modules/lavalandmob/icons/mob.dmi'
      icon_state = "dead_1"
      sound = 'modular_armok/modules/lavalandmob/sounds/boom.ogg'
  ```
...
Example: `code/modules/mob/living/living.dm` -> `modular_armok/master_files/code/modules/mob/living/living.dm`
...
In short, most of the modular code will be placed in the subfolders of your main module folder **`modular_armok/modules/yourmodule/code/`**, with similar rules as with the assets. Do not mirror core code folder structures inside your modular folder.

For example, `modular_armok/modules/xenoarcheaology/code` containing all the code, tools, items and machinery related to it.
...
In general they are a good idea and encouraged whenever it is possible to do so. However this is not a hard rule, and sometimes Armok edits are preferable. Just try to use your common sense about it.
...
Performance should also be considered when you are overriding a hot proc (like Life() for example), as each additional call adds overhead. Armok edits are much more performant in those cases. For most procs this won't be something you have to think about, though.
...
- **Addition:**
  ...
  ```byond
  //ARMOK EDIT ADDITION BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
...
- **Removal:**
  ...
  ```byond
  //ARMOK EDIT REMOVAL BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  /*
  ...
  */
  ...
  ```byond
  //ARMOK EDIT REMOVAL BEGIN - SHUTTLE_TOGGLE - (Moved to modular_armok/shuttle_toggle/randomverbs.dm)
  /*
...
- **Change:**
  ...
  ```byond
  //ARMOK EDIT CHANGE BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  //if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE) - ARMOK EDIT - ORIGINAL
  if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE, SHUTTLE_DISABLED)
  //ARMOK EDIT CHANGE END
      return 1
  ```
...
That folder is **`code/__DEFINES/~armok_defines`**, in which you can add them to the existing files, or create those files as necessary.
...
- Good: /modular_armok/modules/example_module/code/disease_mob.dm
- Bad: /modular_armok/modules/example_module/code/modules/antagonists/disease/disease_mob.dm

**Icons**: Any .DMI files must go in here.

- Good: /modular_armok/modules/example_module/icons/mining_righthand.dmi
- Bad: /modular_armok/modules/example_module/icons/mob/inhands/equipment/mining_righthand.dmi
...
ALL of the tgui files are located in `/tgui/packages/tgui/interfaces` and its subdirectories; there is no specific folder for Armok UIs.
...
You can do both `// ARMOK EDIT` and `/* ARMOK EDIT */`, though in some cases you may have to use one over the other.
...
```js
// THIS IS A ARMOK UI FILE
```

This way they are easily identifiable as modular TGUI .tsx/.jsx files. You do not have to do anything further, and there will never be any need for a Armok edit comment in a modular TGUI file.
